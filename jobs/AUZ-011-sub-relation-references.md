...
### [Implementer] Arrow chain reaching a userset works for the common-case Check (verification gap closed in patch)

After v1.5.0 shipped, a review surfaced that the schema fixture didn't exercise an arrow permission chaining through a userset on the right-side resource (e.g. `permission inherited_collab = parent->mixed_browse` where the parent's `mixed_browse` reaches `team#admin`). The codegen path was implemented (the arrow walker in `permissionInputUsersets` deliberately does NOT recurse through right-side permissions because their usersets live on a different resource — they're not direct subjects of THIS resource's Check call), but no e2e test verified that SpiceDB walks the chain server-side correctly when called with a user input. Closed by adding `permission inherited_collab = parent->mixed_browse` on `extsvc/document` and 3 e2e tests: direct user on parent grants; user reachable via arrow → userset → owner chain grants; unrelated user denies. All 3 pass first-try with the v1.5.0 codegen unchanged. Confirms the design decision to expose only direct-subject inputs on arrow Check methods — the userset-as-input rare case is reserved for permissions where the userset is reachable WITHOUT crossing a resource boundary.
