...
- **Uniform builtin signatures** — Check is 3-arg `(subject object, resource_id string, caveat_context object) → bool`; Lookup is 2-arg `(subject object, caveat_context object) → []string`. `subject` is an object keyed by SpiceDB subject type, value a single id string or a list of id strings — `{"extsvc/user": "alice"}` or `{"extsvc/user": ["a","b"], "extsvc/group": ["g"]}`. Check is AND across present keys (every present subject type must be granted, matching the typed `Check<X>` method); Lookup uses the first present key. `caveat_context` is always required at the call site; pass `{}` when no caveat applies.
...
- **`RegisterSpiceDBBuiltinsGlobal(engine, ctx)`** (AUZ-021) — a second exported function in each generated `opa.gen.go`, alongside `SpiceDBBuiltins`. Registers the SpiceDB builtins into OPA's process-global registry (`ast.Builtins` + the topdown function map) via `rego.RegisterBuiltin2/3`, so `runtime.NewRuntime` and the standard `/v1/data` endpoint resolve them — the per-instance `[]func(*rego.Rego)` form is invisible to the runtime, which builds its compiler at construction time from the global builtin universe. **Call once at startup before any concurrent compilation** — `ast.RegisterBuiltin` is not concurrency-safe; double-calling appends duplicate `ast.Builtins` entries. The per-instance `SpiceDBBuiltins()` remains the no-global-state choice for `rego.New` callers. Both functions reuse the same per-Check/per-Lookup decl + closure fragments (shared `{{ define }}` template blocks).
- **Generated fixtures** — `example/authzed/{bookingsvc,menusvc,extsvc}/opa.gen.go` committed (round-trip regression bar); each now declares both `SpiceDBBuiltins` and `RegisterSpiceDBBuiltinsGlobal`.
- **`example/opa-embed/` — runnable all-embedded demo** (AUZ-020, rewritten in AUZ-021). Single Go binary composing SpiceDB (testcontainers, MemDB) + **OPA's runtime** (`runtime.NewRuntime` — the standard OPA server) + the generated `RegisterSpiceDBBuiltinsGlobal`, serving policy decisions over OPA's standard HTTP API (`POST /v1/data/authz/allow`, `GET /v1/policies`, `GET /health`). Ships a sample `policy.rego` mixing RBAC + ReBAC + a deny override, and a README with run + curl recipes + an ASCII architecture diagram. Docker required (SpiceDB testcontainer). The AUZ-020-era plain-`net/http` stopgap is gone — once `RegisterSpiceDBBuiltinsGlobal` existed, the demo could use OPA's real server.
...
- **Subject-argument format** (AUZ-022): a keyed object `{"<namespace>": id | [id, ...]}` whose keys mirror the codegen's own typed `Check<X>Inputs{User, Group, Role}` shape — not a `"type:id"` string. (The string form, present in the unreleased-1.14.0 work, never shipped externally; AUZ-022 replaced it before release.) The object is typed as fully-dynamic — static per-permission key sets were tried and rejected because OPA's static object properties are effectively required-all, which would break single-key calls; a bogus subject type surfaces as a runtime error from SpiceDB's `CheckPermission` rather than a compile-time error. The value type *is* compile-time checked (non-string/non-array fails the OPA type checker). The OPA binding calls `Engine.CheckPermission` per present key (AND), bypassing the typed `Check<X>` method's struct layer.
